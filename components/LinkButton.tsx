...
const LinkButton: React.FC<LinkButtonProps> = ({ href, onClick, children, disabled = false, className = '' }) => {
  const baseClasses =
    'shine-hover relative flex items-center justify-center w-full max-w-md text-center text-xs sm:text-sm font-medium tracking-wider bg-black/30 text-stone-100 rounded-lg px-3 py-2.5 sm:px-5 sm:py-3 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-amber-300 border border-amber-300/20';
